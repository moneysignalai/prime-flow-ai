...
def _format_time(signal: Signal) -> str:
    """Return a human-friendly time stamp (HH:MM:SS ET)."""
    ...
    if ts is None:
        return "Time: n/a"
    return f"Time: {ts.strftime('%H:%M:%S')} ET"  # TODO: confirm timezone; assuming ET for alerts
...
def _format_flow_summary(signal: Signal) -> str:
    ...
    return (
        f"{event.contracts}x @ ${event.option_price:.2f} | Strike {event.strike} exp {event.expiry.isoformat()} | "
...
def _format_microstructure(signal: Signal) -> str:
...
def _format_intraday_structure(signal: Signal) -> str:
    price_line = _format_price_context(signal)
    price_info = signal.context.get("price_info", {}) if isinstance(signal.context, dict) else {}
...
def format_medium_alert(signal: Signal) -> str:
    ...
    lines = [
        ...
        _format_flow_summary(signal),
        flow_intent,
        _format_intraday_structure(signal),
        why_good,
        risk,
        _format_regime(signal),
        _format_time(signal),
